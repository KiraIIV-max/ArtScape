...
const Artist = () => {
  ...
  const refreshArtworks = async () => {
    ...
    try {
      ...
      // If the response carries an artist status, honor it (keeps client state in sync)
      const statusFromData =
        (data.artist?.verification_status || data.artist?.status) ||
        (artworksArray[0]?.artist?.verification_status ||
          artworksArray[0]?.artist?.status);
      if (statusFromData) {
        setArtistStatus(statusFromData);
      }
    } catch (err) {
  ...
  // Drop the /artist/profile fetch to avoid the 404; we now rely on cached status and on any status shipped with artworks.
  useEffect(() => {
    refreshArtworks();
  }, [headers]);
  ...
  const handleDeleteArtwork = async (artworkId) => {
    ...
    try {
      const res = await fetch(
        `${API_BASE_URL}/artist/artworks/${artworkId}`,
        {
          method: "DELETE",
          headers,
        }
